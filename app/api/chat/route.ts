...
export async function POST(req: Request) {
  const { messages, mode, grid, meta, versionNumber, designId, flow, existingPublishedDoc, pathwayTitle, contributorOrg } = await req.json();
  ...
  if (mode === 'analysis-doc') {
    ...
    systemPrompt = pathwayDraftSystemPrompt(
      ...
      typeof contributorOrg === 'string' ? contributorOrg : null
    );
