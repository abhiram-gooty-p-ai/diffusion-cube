...
import { adoptionResourcePrefix, adopterPrefix, getS3FileClient, getS3FileConfig } from '@/lib/s3-files';
...
export async function POST(request: Request) {
  ...
  const adoptionName = typeof (design.meta as { name?: unknown } | null)?.name === 'string'
    ...
    : 'adoption';
  const expectedPrefix = `${adoptionResourcePrefix(adopterPrefix({ id: user.id, name, email: user.email }), adoptionName, design.id)}/`;
