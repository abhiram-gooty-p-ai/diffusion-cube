...
1. The Pathway Framework (defines the four dimensions, cross-cutting concerns, four stages, five unit types, the question bank's insight forms for each dimension×stage cell, and the pathway document's output structure — Sections 0–6 plus the unnumbered Provenance appendix row).
...
- Provenance appendix — the table's unnumbered final row, contributor-facing only, never surfaced in adopter-facing output. It is not "Section 7" and must not be numbered as a continuation of 0–6.
...
These rules govern how to write each section correctly. They assume you already have the Framework's Section 0–6 definitions and the Provenance appendix row in front of you — they don't repeat what each section is for, only the execution details an LLM needs that aren't naturally expressed as a structural definition.
...
Apply the four gap-survival tests before including any gap (concreteness, non-fabrication, boundary-restatement, existing-coverage), and default toward dropping over reframing. Ground every surviving gap in this pathway's own vocabulary and cite the specific unit number(s) it relates to.
...
- A bulleted tag block immediately below the title:
  ...
  - `Relevant to: ...` — optional, include only when the source material itself differentiates by audience (e.g. explicitly separates guidance for government vs. technical vs. social-sector readers). Do not add this to every unit as decoration.
- Then bulleted content fields, matched to the type:
  - Strategic/Tactical Decision: Decision, Alternative considered, Condition — applies when, Condition — fails when (if stated), Before → After (or "Not documented in the source" if genuinely absent).
...
Table listing every unit tagged Toolkit Asset or Playbook, cross-referenced by unit number, with a one-line reuse condition each.

### Section 5 — Problem→solution patterns

Table: Problem | Root cause | Solution | Result | Condition. Populate from Failure-and-Fix units and any other clear problem→fix pattern in the source material — don't force entries that aren't genuinely there.
...
### Provenance appendix (the Framework table's unnumbered final row)
